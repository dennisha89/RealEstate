...
export function estimateRent(
  ...
  if (comps.length === 0) {
    ...
    return {
      ...
      adjustments: [],
      pricePerSqft: {
        low: Math.round((baseRent * 0.9) / property.sqft * 100) / 100,
        mid: Math.round(baseRent / property.sqft * 100) / 100,
        high: Math.round((baseRent * 1.1) / property.sqft * 100) / 100,
      },
    };
  }
  ...
  return {
    ...
    adjustments,
    pricePerSqft: {
      low: Math.round((adjustedRent * 0.92) / property.sqft * 100) / 100,
      mid: Math.round(adjustedRent / property.sqft * 100) / 100,
      high: Math.round((adjustedRent * 1.08) / property.sqft * 100) / 100,
    },
  };
}
