...
- **34 engines** in `property-analyzer/lib/engines/`, **60 engines** in `lootvue/src/lib/engines/`:
  - 39 root engines (includes market-forecast, leading-indicator, bubble-detection, capital-flow-composite, municipal-prediction)
  - 16 confluence engines in `confluence/` (12 domain + correlation-matrix + master + orchestrator)
  - 4 cross-domain confluences in `confluence/cross-domain/` (CHRONOS, RIPPLE, MIRROR, ECHO)
  - 1 oracle engine in `oracle/` (prediction-tracker)
