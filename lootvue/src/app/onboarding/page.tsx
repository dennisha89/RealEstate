...
import { ArrowRight, ArrowLeft, User, Target, Search, Building2, Check, Plus, Trash2, Sparkles, BarChart3 } from "lucide-react";
...
const PILL_ACCENT = "border-gold/40 bg-gold-muted text-gold-light font-medium";
...
const SLIDER_ACCENT = "w-full accent-gold h-1.5 bg-surface-muted rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-gold-light [&::-webkit-slider-thumb]:shadow-glow-gold [&::-webkit-slider-thumb]:cursor-pointer";
  ...
  if (done) return (
    <div className="min-h-screen bg-surface flex items-center justify-center px-4">
      <div className="text-center animate-scale-in">
        ...
        <h1 className="text-2xl font-display font-bold text-content-primary mb-2">Your LootVue is ready.</h1>
    ...
    <div className="min-h-screen bg-surface flex flex-col">
      ...
      <header className="relative z-10 px-6 pt-8 pb-4 max-w-lg mx-auto w-full">
        <div className="flex items-center gap-2 mb-8">
          ...
          <span className="font-display font-bold text-content-primary tracking-[0.08em] text-sm">LOOTVUE</span>
        </div>
        <div className="flex items-center gap-2 mb-2">
          {Array.from({ length: STEPS }).map((_, i) => (
            <div key={i} className="flex-1 h-1 rounded-full overflow-hidden bg-surface-muted">
              <div className="h-full rounded-full bg-gradient-gold transition-all duration-500" style={{ width: i < step ? "100%" : i === step ? "50%" : "0%" }} />
            </div>
          ))}
        </div>
      ...
      <main className="relative z-10 flex-1 flex items-start justify-center px-6 pb-32">
        <div className="w-full max-w-lg animate-fade-in" key={step}>
          ...
          {step === 0 && <div className="space-y-6">
            <div>
              <div className="flex items-center gap-2 text-gold-light mb-3"><User className="w-4 h-4" /><span className="text-xs font-semibold uppercase tracking-[0.1em]">Investor Profile</span></div>
              <h1 className="text-2xl font-display font-bold text-content-primary tracking-tight mb-2">Welcome to LootVue. Let&apos;s set up your investment profile.</h1>
            ...
            <div>
              ...
              <div className="space-y-2">
                {INVESTOR_TYPES.map((t) => (
                  <button key={t.v} onClick={() => s.setInvestorType(t.v)} className={`${optCard} ${s.investorType === t.v ? "!border-gold/40 !bg-gold-muted" : ""}`}>
                    <div className={`text-sm font-medium ${s.investorType === t.v ? "text-gold-light" : "text-content-primary"}`}>{t.l}</div>
          ...
          {step === 2 && <div className="space-y-6">
            <div>
              <div className="flex items-center gap-2 text-gold-light mb-3"><Search className="w-4 h-4" /><span className="text-xs font-semibold uppercase tracking-[0.1em]">Buy Box</span></div>
            ...
            <div>
              ...
              {s.buyBox.targetMarkets.length > 0 && <div className="flex flex-wrap gap-1.5 mt-2">
                {s.buyBox.targetMarkets.map((m) => <span key={m} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-gold-muted text-gold-light text-xs font-medium">{m}<button onClick={() => s.setBuyBox({ targetMarkets: s.buyBox.targetMarkets.filter((x) => x !== m) })} className="hover:text-white transition-colors"><Trash2 className="w-3 h-3" /></button></span>)}
              </div>}
            ...
            <div>
              ...
              <div className="flex items-center justify-between text-xs text-content-disabled mb-1.5">
                <span>4%</span><span className="text-lg font-mono font-bold text-gold-light">{s.buyBox.minCapRate}%</span><span>10%</span>
              </div>
          ...
          {step === 3 && <div className="space-y-6">
            ...
            {imp === null && <div className="space-y-2">
              {([["add", "Yes, I'll add them now", "Add your current properties so LootVue can track your portfolio"], ["later", "I'll add them later", "You can always add properties from your dashboard"], ["first", "I'm looking for my first property", "We'll help you find the right one"]] as const).map(([k, t, d]) => (
            ...
            {imp === "first" && <div className="p-5 rounded-lg bg-emerald-muted/30 border border-emerald/10 text-center">
              ...
              <p className="text-xs text-content-secondary">LootVue will walk you through every step &mdash; from finding markets to analyzing your first deal with confidence.</p>
            </div>}
          </div>}
      ...
      <footer className="fixed bottom-0 left-0 right-0 z-20 border-t border-surface-border glass-subtle px-6 py-4">
        <div className="max-w-lg mx-auto flex items-center justify-between">
          ...
          {step < STEPS - 1
            ...
            : <button onClick={finish} className="btn-emerald">Launch LootVue <ArrowRight className="w-4 h-4" /></button>}
        </div>
      </footer>
    </div>
  );
