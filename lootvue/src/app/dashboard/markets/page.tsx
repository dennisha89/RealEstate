...
// z-score approximations for each signal direction (for visual bars)
function zForDir(dir: "bullish" | "bearish" | "neutral"): number {
...
export default function MarketsPage() {
  ...
  const selectedMarket = useMemo(
    () => SAMPLE_MARKET_DATA.find((m) => m.stateCode === activeState) ?? null,
    [activeState]
  );

  const sortedData = useMemo(() => {
    return [...SAMPLE_MARKET_DATA].sort((a, b) => {
  ...
  const tickerData = [...SAMPLE_MARKET_DATA, ...SAMPLE_MARKET_DATA];
  ...
  const nationalBullish = Math.round(
    SAMPLE_MARKET_DATA.filter((m) => scoreSignal(m.score) === "BUY").length
  );
  ...
  return (
    <div className="flex flex-col gap-4 p-4 min-h-full bg-surface">
      ...
      <div className="flex items-start justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="page-subtitle">
            ...
            {" "}strongest predictor · {SAMPLE_MARKET_DATA.length} tracked markets
          </p>
        </div>
        <GeoBreadcrumb />
      </div>
      ...
      {/* ─── Ticker Bar ────────────────────────────────────────────────────────── */}
      <div
        className="relative overflow-hidden rounded-xl border border-surface-border bg-surface-card"
        style={{ height: 42 }}
        ...
        >
          <InteractiveMap
            ...
            onGeoSelect={(level, code) => {
              if (level === "state") {
                const found = SAMPLE_MARKET_DATA.find((m) => m.stateCode === code);
