...
export default function DealRoomDetailPage() {
  ...
  const room         = useDealRoomStore((s) => s.getRoomByToken(token));
  ...
  const [commentAuthor, setCommentAuthor] = useState("Investor");
  const [interestRole, setInterestRole]   = useState<Role>("buyer");
  const [interestName, setInterestName]   = useState("Investor");
  ...
  const r = room.analysis as AnalysisResult;
