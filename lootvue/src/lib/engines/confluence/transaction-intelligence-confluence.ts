...
export function computeTransactionIntelligence(input: TransactionIntelligenceInput): TransactionIntelligenceResult {
  ...
  const top = L[topEntry[0]], bot = L[botEntry[0]];

  let thesis: string;
  if (confluenceScore >= 60) {
    thesis = `${verdict.replace(/_/g, " ")} (${confluenceScore}/100). ${top} leads at ${topEntry[1].score}. `
    ...
    thesis = `${verdict.replace(/_/g, " ")} (${confluenceScore}/100). ${bot} at ${botEntry[1].score} `
