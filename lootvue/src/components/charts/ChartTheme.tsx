...
/**
 * Shared chart constants for LootVue light theme.
...
export function ChartTooltipContent({
  ...
  return (
    <div style={TOOLTIP_STYLE}>
      {label && (
        <p style={{ fontSize: 10, color: "#6B7280", marginBottom: 4, fontFamily: "JetBrains Mono, monospace" }}>
          {label}
        </p>
      )}
      {items.map((item) => (
        <div key={item.name} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
          ...
          <span style={{ fontSize: 12, color: "#111111", fontWeight: 600, fontFamily: "JetBrains Mono, monospace" }}>
            {item.value}
          </span>
        </div>
      ))}
...
export function AiInsightCard({
  ...
  return (
    <div className={`card border-l-2 border-l-gold ${className ?? ""}`}>
      <div className="flex items-center gap-2 mb-2">
        <svg className="w-3.5 h-3.5 text-gold" fill="currentColor" viewBox="0 0 24 24">
          <path d="M12 2L9.19 8.63 2 9.24l5.46 4.73L5.82 21 12 17.27 18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2z" />
        </svg>
        <span className="text-xs font-semibold text-gold">{title ?? "AI Insight"}</span>
      </div>
