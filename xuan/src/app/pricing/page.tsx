...
import { Check, X as XIcon, ArrowRight, ChevronDown, ChevronUp, Lock } from "lucide-react";
...
const ANCHORS = [
  ...
  { name: "Xuan Pro", price: "$29", per: "/month" },
];
    ...
    <div className="min-h-screen bg-surface text-content-primary overflow-x-hidden">
      ...
      <section className="relative pt-32 md:pt-40 pb-4 px-6 overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto text-center">
          ...
          <div className="grid grid-cols-3 gap-4 mb-10 max-w-md mx-auto">
            {ANCHORS.map((p, i) => (
              <div key={p.name} className={`p-4 rounded-xl border ${i < 2 ? "border-white/[0.06] bg-white/[0.02]" : "border-accent/30 bg-accent-muted"}`}>
                <div className="text-xs text-content-tertiary mb-2">{p.name}</div>
                <div className={`font-mono font-bold ${i < 2 ? "text-content-disabled line-through" : "text-accent-light text-2xl"}`}>{p.price}</div>
          ...
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <button onClick={() => setAnnual(!annual)} className={`relative w-12 h-6 rounded-full transition-colors duration-200 ${annual ? "bg-accent" : "bg-surface-muted"}`} aria-label="Toggle annual billing">
      ...
      <section className="py-12 px-6">
        <div className="max-w-5xl mx-auto grid md:grid-cols-3 gap-5">
          {TIERS.map((t) => (
            <div key={t.name} className={`relative rounded-xl p-6 border transition-all ${t.hl ? "border-accent/40 bg-surface-card glow-accent" : "border-white/[0.06] bg-surface-card"}`} style={t.hl ? undefined : { boxShadow: "0 1px 3px rgba(0,0,0,0.3), inset 0 0.5px 0 0 rgba(255,255,255,0.04)" }}>
              {t.badge && <span className="absolute -top-3 left-1/2 -translate-x-1/2 badge bg-accent text-white font-bold px-3 py-1 text-[11px]">{t.badge}</span>}
              <div className="mb-6">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/[0.08]">
                    ...
                    <th className="text-center py-4 px-4"><span className="text-accent-light font-bold text-sm">Pro</span></th>
