...
const TIERS: Tier[] = [
  { name: "Diamond", icon: "D", range: "Top 5%", avgCap: "8.1%", avgCashFlow: "$520/door", color: "text-accent-light bg-accent-muted" },
    ...
    <div className="animate-fade-in space-y-6">
            ...
            return (
              <div
                ...
                className={`flex items-center gap-3 p-3 rounded-lg transition-colors ${
                  isCurrent ? "bg-accent-muted/50 border border-accent/20" : "bg-white/[0.02] border border-transparent"
                }`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {isCurrent && <span className="badge-accent">YOU</span>}
                  </div>
                ...
                {isCurrent && <ArrowUpRight className="w-4 h-4 text-accent-light shrink-0" />}
              </div>
            );
          })}
        </div>
      ...
      <section className="card-accent">
        <div className="text-[10px] text-content-disabled uppercase tracking-[0.1em] mb-3 font-medium flex items-center gap-2">
          <Zap className="w-3.5 h-3.5 text-accent-light" /> How To Level Up
        </div>
        <div className="space-y-2.5">
          {LEVEL_UP.map((l) => (
            <div key={l.text} className="flex items-start gap-2.5 p-2.5 rounded-lg bg-white/[0.03]">
              <ChevronRight className="w-3.5 h-3.5 text-accent-light mt-0.5 shrink-0" />
